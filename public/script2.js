// ==================== script2.js (cópia namespaced do seu script.js) ====================
'use strict';

// Base da API
const apiUrl2 = window.location.origin;
console.log('[script2.js] carregado');

// ---------- UI helpers (versão 2) ----------
function showAlert2(message, type = 'success') {
  const alertPlaceholder = document.getElementById('alertPlaceholder2');
  if (alertPlaceholder) {
...
async function fetchJSON2(path) {
  ...
  const res = await fetch(`${apiUrl2}${path}`, {
...
async function carregarUsuariosExternos2() {
  const lista = await fetchJSON2('/usuarios-externos');
  return lista.map(u => ({
    value: u.nome,
    label: u.nome,
    id: u._id,
...
async function carregarContratos2() {
  const lista = await fetchJSON2('/contratos');
  return lista.map(c => ({ value: c.numero, label: c.numero }));
}

// ---------- Instruções por fluxo ----------
const fluxoInstrucoes2 = {
...
function agoraParaDatetimeLocalMin2() {
  const d = new Date();
...
// ---------- UI builders ----------
function buildSelectOptions2(selectEl, options) {
...
// ---------- Form modal (IDs *2) ----------
async function abrirFormulario2(fluxo) {
  const modalTitle = document.getElementById('modalTitle2');
  const modalBody  = document.querySelector('#fluxoModal2 .modal-body');
  ...
  instrucaoText.textContent = fluxoInstrucoes2[fluxo] || 'Preencha todos os campos.';
  modalBody.appendChild(instrucaoText);

  const fluxoForm = document.createElement('form');
  fluxoForm.id = 'fluxoForm2';
  ...
  try {
    if (fluxo === 'Consultar empenho') {
      ...
      campos = [
        ...
        { id: 'dataFormatada', placeholder: 'Data', type: 'date', value: hojeYYYYMMDD2() },
    ...
    console.error('Erro ao montar campos do fluxo (v2):', e);
    showAlert2('Falha ao carregar dados para o formulário.', 'danger');
    return;
  }
  ...
  campos.forEach((campo) => {
    ...
    if (campo.type === 'select') {
      ...
      buildSelectOptions2(input, campo.options || []);
      ...
      // listeners do método de upload
      input.addEventListener('change', function(e) {
        const metodo = e.target.value;
        const imagensContainer = document.getElementById('imagensContainer2');
        const zipContainer = document.getElementById('zipContainer2');
        const pdfContainer = document.getElementById('pdfContainer2');
        const zipInput = document.getElementById('arquivoZip2');
        const numeroImagensSelect = document.getElementById('numeroImagens2');
        const pdfInput = document.getElementById('arquivoPdf2');
        ...
        if (metodo === 'Imagens Individuais') {
          ...
          document.querySelectorAll('#arquivosContainer2 input[type="file"]').forEach(inp => inp.required = true);
          ...
          if (pdfInput) { pdfInput.required = false; pdfInput.value = ''; }
          document.querySelectorAll('#arquivosContainer2 input[type="file"]').forEach(inp => { inp.required = false; inp.value = ''; });
  ...
  // ----- Containers extras (IDs *2 para não conflitar) -----
  // Imagens individuais
  const imagensContainer = document.createElement('div');
  imagensContainer.id = 'imagensContainer2';
  imagensContainer.style.display = 'none';
  ...
  numeroImagensSelect.addEventListener('change', function() {
    ...
    const arquivosContainer = document.getElementById('arquivosContainer2');
  ...
  arquivosContainer.id = 'arquivosContainer2';
  imagensContainer.appendChild(arquivosContainer);

  // ZIP
  const zipContainer = document.createElement('div');
  zipContainer.id = 'zipContainer2';
  zipContainer.style.display = 'none';
  ...
  const rImediatoId = 'envio_imediato2';
  const rAgendarId  = 'envio_agendar2';
  ...
  agGroup.addEventListener('change', (e) => {
    if (e.target && e.target.name === 'envio') {
      ...
      if (isAgendar && !quandoInput.min) quandoInput.min = agoraParaDatetimeLocalMin2();
    }
  });
  ...
  fluxoForm.onsubmit = enviarFormularioAxios2;

  // Abre modal 2
  try { $('#fluxoModal2').modal('show'); } catch(e){ console.error(e); }
}

// ---------- Submit (axios) ----------
function enviarFormularioAxios2(e) {
  e.preventDefault();

  // valida agendamento
  const envioSelecionado = (e.target.querySelector('input[name="envio"]:checked') || {}).value || 'imediato';
  if (envioSelecionado === 'agendar') {
    const quandoEl = e.target.querySelector('#quando2');
    if (!quandoEl || !quandoEl.value) { showAlert2('Informe a data e hora do agendamento.', 'warning'); return; }
    const min = quandoEl.min ? new Date(quandoEl.min) : null;
    const escolhido = new Date(quandoEl.value);
    if (min && escolhido < min) { showAlert2('Escolha um horário no futuro (mínimo alguns minutos à frente).', 'warning'); return; }
  }

  showLoadingOverlay2();

  const fluxo = document.getElementById('modalTitle2').innerText;
  ...
  if (envioSelecionado === 'agendar') {
    const whenEl = e.target.querySelector('#quando2');
    if (whenEl && whenEl.value) formData.append('quandoUtc', new Date(whenEl.value).toISOString());
  }
    ...
    if (['Unir PDFs', 'PDF para JPG', 'Dividir PDF', 'PDF pesquisável (OCR)'].includes(fluxo)) {
      ...
      if (!filename) {
        if (fluxo === 'Unir PDFs') {
          ...
          filename = first.replace(/\.[^.]+$/, '') +analysis code_execution 여부
